...
Поиск по маркерам детерминирован и лёгок, но не заменяет универсальный семантический поиск. Важны опорные слова запроса и качество загрузки данных; стандартный recall ранжирует кандидатов, а не отказывается от слабых совпадений автоматически. Если хосту агента лучше ничего не получить, чем получить слабую память, можно включить `--min-score` / `min_score`. Отдельно приведены [измерения и их ограничения](docs/RELEASE.md#external-retrieval-evidence).

## Результаты измерений

Локальные retrieval-прогоны в release-сборке используют строгий top-5 и официальные файлы наборов данных, которые не входят в репозиторий:

| Набор данных / стандартный focused-путь Exact | Записей | Hit@5 | Recall@5 | MRR@5 |
|---|---:|---:|---:|---:|
| LongMemEval Oracle | 4 578 | 0.972 | 0.877 | 0.775 |
| LongMemEval-S | 85 253 | 0.896 | 0.782 | 0.701 |
| LoCoMo | 5 881 | 0.531 | 0.487 | 0.393 |

Это оценка retrieval через детерминированные адаптеры, а не качества итогового ответа LLM или превосходства над другим продуктом. Ревизии и контрольные суммы наборов, правила загрузки, диагностическое сравнение с BM25, размеры контекста и ограничения описаны в [документации по выпуску](docs/RELEASE.md#external-retrieval-evidence).
