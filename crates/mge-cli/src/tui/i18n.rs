...
pub fn tr(language: Language, key: TKey) -> &'static str {
    match language {
        Language::En => match key {
            ...
            TKey::ImportPath => "Markdown path",
            TKey::ImportScope => "Import scope",
            TKey::ImportedFiles => "Imported files",
            TKey::ImportedCells => "Imported cells",
            TKey::ExportPath => "Export path",
            TKey::Language => "Language",
            TKey::SettingsPersistenceNote => {
                "Index kind is stored persistently; language and default recall mode apply to this TUI session."
            }
            TKey::HelpText => {
        ...
        Language::Ru => match key {
            ...
            TKey::ImportPath => "Путь к Markdown",
            TKey::ImportScope => "Scope импорта",
            TKey::ImportedFiles => "Импортировано файлов",
            TKey::ImportedCells => "Импортировано записей",
            TKey::ExportPath => "Путь экспорта",
            TKey::Language => "Язык",
            TKey::SettingsPersistenceNote => {
                "Тип индекса сохраняется в store; язык и recall mode действуют в текущей TUI-сессии."
            }
            TKey::HelpText => {
