...
The harness also exposes `--mge-rankers lexical-experiments`, `--max-context-bytes`, `--max-results-per-session`, and `avg_candidate_items` for explicit candidate/output-budget experiments. A full-term-frequency RRF improved LoCoMo four-turn chunks (Hit@5 `0.7365` to `0.7481`) and LongMemEval Oracle (`0.9723` to `0.9766`), but regressed raw-turn LoCoMo (`0.5331` to `0.5321`), so it was not promoted. A 2,800-byte memory-content budget reduced LoCoMo chunk context by about 23% but also reduced Recall@5 from `0.6859` to `0.6726`; one-result-per-session fell to `0.5986`. These policies remain development measurements rather than production defaults.
