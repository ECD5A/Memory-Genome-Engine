...
echo "Building debug binaries for smoke..."
...
find_bin() {
  ...
  if [[ -x "$bin_dir/$name" ]]; then
    ...
    echo "missing debug binary: $name" >&2
    return 1
  fi
}
...
echo "MCP smoke..."
printf '{"jsonrpc":"2.0","id":1,"method":"mge_stats","params":{"store_path":"%s"}}\n' "$plain_store" \
  | "$mcp_bin" \
  | grep -q '"protocol_version":"mge-jsonrpc-1"'
