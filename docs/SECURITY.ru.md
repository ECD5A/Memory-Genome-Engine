...
Мандат 3 закрывается как payload encryption ready для текущего продукта. Future work является non-blocking и должен начинаться только с отдельного design:
